...
const Home = () => {
  return (
    <>
      <section className="overflow-hidden px-4 sm:px-6 m-10 sm:flex sm:flex-col gap-4 md:justify-center md:items-center">
        <TitleSection
          pill="Your Workspace Perfected"
          title="All-in-One Collaboration and Productivity Platform"
        />
        <div className="bg-white p-[2px] mt-6 rounded-xl  sm:w-[300px] bg-gradient-to-r from-primary to-brand-primaryBlue">
          <Button
      ...
      <section className="mt-20 px-4 sm:px-6">
        <TitleSection
          ...
          pill="Pricing"
        />
        <div className="flex flex-col-reverse sm:flex-row gap-4 justify-center sm:items-start items-center mt-10">
            {PRICING_CARDS.map((card)=>(
                <CustomCard  key={card.planType} className={clsx('w-[300px] rounded-2xl dark:bg-black/40 backdrop-blur-3xl relative',
                {
                ...
                cardHeader = {<CardTitle className="text-2xl font-semibold">
                    ...
                    <div className="hidden dark:block w-full blur-[120px] rounded-full h-32 absolute bg-brand-primaryPurple/80 -z-10 top-0">
                        <Image src={Diamond} alt="Pro-Plan Icon" className="top-6 right-6 absolute" />
                    </div>
                    </>}
                    {card.planType}
                </CardTitle>
            ...
            cardContent = {
                ...
                {+card.price > 0 ? (<span className=" text-washed-purple-800 ml-1">/mo</span>): ("")}
